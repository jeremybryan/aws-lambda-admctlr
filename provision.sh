...
aws --profile "$PROFILE" lambda create-function --function-name adminctlr --zip-file fileb://function.zip --handler index.handler --runtime nodejs12.x --role "$ROLEARN"
...
echo "Using integration id $INT_ID"
...
aws --profile "$PROFILE" lambda add-permission --statement-id adminctlr-id --action lambda:InvokeFunction --function-name "$FUNCTARN" --principal apigateway.amazonaws.com --source-arn "$EXECUTE_API"
